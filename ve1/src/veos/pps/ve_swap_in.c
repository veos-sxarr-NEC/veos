 ...
 */
void
ve1_pps_veos_restore_registered_memory(struct ve_task_struct *group_leader,
				struct ve_swapped_virtual *vrt, pgno_t pno)
{
		ve1_dmaatb_reg_t *group_leader_p_ve_mm_dmaatb = &((ve1_veos_dmaatb_reg_t *)group_leader->p_ve_mm->dmaatb)->dmaatb_reg;
...
VEOS_ARCH_DEP_FUNC(ve1, arch_pps_veos_restore_registered_memory, ve1_pps_veos_restore_registered_memory)
 ...
 */
int
ve1_pps_allocate_new_dmaatb_dir(struct ve_task_struct *tsk)
{
	int retval;
	...
	ve1_veos_dmaatb_reg_t ve1_new_process_soft_copy;
	new_process_soft_copy = (veos_dmaatb_reg_t *)&ve1_new_process_soft_copy;
	ve1_dmaatb_reg_t *ve1_new_process_soft_copy_dmaatb = &ve1_new_process_soft_copy.dmaatb_reg;
	...
	for(old_dir = vnode->nr_avail_cores; old_dir < VE1_DMAATB_DIR_NUM;
								old_dir++) {
		...
		memcpy(&(ve1_new_process_soft_copy_dmaatb->dir[new_dir]),
			&(tsk_p_ve_mm_dmaatb->dir[old_dir]), sizeof(uint64_t));
		memcpy(ve1_new_process_soft_copy_dmaatb->entry[new_dir],
	...
	for (new_dir = vnode->nr_avail_cores; new_dir < VE1_DMAATB_DIR_NUM;
								new_dir++) {
		memcpy(&(tsk_p_ve_mm_dmaatb->dir[new_dir]),
			&(ve1_new_process_soft_copy_dmaatb->dir[new_dir]),
			sizeof(uint64_t));
		memcpy(tsk_p_ve_mm_dmaatb->entry[new_dir],
			ve1_new_process_soft_copy_dmaatb->entry[new_dir],
			sizeof(uint64_t) * VE1_DMAATB_ENTRY_MAX_SIZE);
	...
	for (new_dir = vnode->nr_avail_cores; new_dir < VE1_DMAATB_DIR_NUM;
		...
		VE1_NODE_ARCH_DEP(VE_NODE(0))->hw_dma_map[new_dir].count = valid_ent;
...
VEOS_ARCH_DEP_FUNC(ve1, arch_pps_allocate_new_dmaatb_dir, ve1_pps_allocate_new_dmaatb_dir)
 ...
 * */
int
ve1_veos_get_pgmod_by_vehva(uint64_t vehva)
{
	int pgmod = 0;
...
VEOS_ARCH_DEP_FUNC(ve1, arch_veos_get_pgmod_by_vehva, ve1_veos_get_pgmod_by_vehva)
 ...
 * */
static int
ve1_veos_reserve_dmaatb_dir(struct ve_task_struct *tsk, uint64_t vehva, int pgmod)
{
	int new_dir;
	int retval = -1;
	int ret = -1;
	...
	if (!ps_isvalid(&(vnode_dmaatb->dir[new_dir]))) {
		...
		memcpy(&(tsk_p_ve_mm_dmaatb->dir[new_dir]),
			&(ve1_new_process_soft_copy_dmaatb->dir[new_dir]), sizeof(uint64_t));
	}
 ...
 * */
int
ve1_veos_swap_in_user_veshm(struct ve_task_struct *tsk)
{
	...
	list_for_each_safe(list_p, list_n, &resource->swapped_attach_veshm_list) {
		...
		for (i = 0; i < count; i++) {
			...
			new_dir = ve1_veos_reserve_dmaatb_dir(tsk, vehva_new_dir, pgmod);
...
VEOS_ARCH_DEP_FUNC(ve1, arch_veos_swap_in_user_veshm, ve1_veos_swap_in_user_veshm)
