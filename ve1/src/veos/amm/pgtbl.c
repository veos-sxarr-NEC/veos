...
/**
 * @file pgtbl.c
 * @brief VE1 dependent part of page table structure
 *
 */
#include <libved.h>
#include <ve_hw.h>
...
static void ve1__dump_dmaatb(FILE *fp, const ve1_dmaatb_reg_t *dmaatb)
{
	int dir_cntr = 0;
	int entry_cntr = 0;
	...
	for (dir_cntr = 0; dir_cntr < VE1_DMAATB_DIR_NUM; dir_cntr++) {
		...
		if (ps_isvalid(&(dmaatb->dir[dir_cntr]))) {
			...
			for (entry_cntr = 0;
				entry_cntr < VE1_DMAATB_ENTRY_MAX_SIZE;
				entry_cntr++) {
#ifdef DEBUG
				fprintf(fd, "page_no \n%d\n", entry_cntr);
#endif
...
void ve1_amm__dump_dmaatb(FILE *fp, const veos_dmaatb_reg_t *ptbl)
{
	const ve1_veos_dmaatb_reg_t *ptr = (const ve1_veos_dmaatb_reg_t *)ptbl;
	ve1__dump_dmaatb(fp, &ptr->dmaatb_reg);
	
}
VEOS_ARCH_DEP_FUNC(ve1, arch_amm__dump_dmaatb, ve1_amm__dump_dmaatb)

static void ve1__validate_dma_pgd(ve1_dmaatb_reg_t *dmaatb, dir_t dir,
				int psnum, int jid, int pgmod)
{
	ve1_atb_dir_t *pgd = &dmaatb->dir[dir];
	ve1_atb_entry_t *pte = dmaatb->entry[dir];
	...
	if (pgmod != PG_4K) {
		...
		ve1_atb_dir_t *pgd_node = NULL;
		/*Will be used in case of DMAATB only*/
		ve1_atb_entry_t *pte_node = NULL;
		struct ve_node_struct *vnode = VE_NODE(0);

		/* partial space directory */
		pgd_node = &((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg.dir[dir];
		pte_node = ((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg.entry[dir];
		...
		for (int ps_entry = 0; ps_entry < VE1_DMAATB_ENTRY_MAX_SIZE;
			ps_entry++)
			pg_invalid(&pte_node[ps_entry]);
                ...
                for (int ps_entry = 0; ps_entry <
                                VE1_ATB_ENTRY_MAX_SIZE; ps_entry++)
                        pg_invalid(&pte[ps_entry]);

}

void ve1_amm__validate_dma_pgd(veos_dmaatb_reg_t *dmaatb, dir_t dir,
				int psnum, int jid, int pgmod)
{
	ve1_veos_dmaatb_reg_t *ptr = (ve1_veos_dmaatb_reg_t *)dmaatb;
	ve1__validate_dma_pgd(&ptr->dmaatb_reg, dir, psnum, jid, pgmod);
}
VEOS_ARCH_DEP_FUNC(ve1, arch_amm__validate_dma_pgd, ve1_amm__validate_dma_pgd)

dir_t ve1_amm__get_dma_pgd(vehva_t vaddr, int jid, bool inval)
{
	struct ve_node_struct *vnode = VE_NODE(0);
	int noc = vnode->nr_avail_cores;
	dir_t ps_entry = vaddr < VEHVA_DIR0_END ? 0 : noc;
	int atb_num = vaddr < VEHVA_DIR0_END ? noc : VE1_DMAATB_DIR_NUM;
	ve1_atb_dir_t *atbp = &((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg.dir[ps_entry];
	int unused = -1;
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm__get_dma_pgd, ve1_amm__get_dma_pgd)

void ve1_amm_invalidate_dmaatb(veos_dmaatb_reg_t *veos_dmaatb)
{
	dir_t dirno = 0;
	ent_t entry = 0;
	ve1_dmaatb_reg_t *dmaatb = &((ve1_veos_dmaatb_reg_t *)veos_dmaatb)->dmaatb_reg;
	VEOS_TRACE("invoked");
	VEOS_DEBUG("dmaatb(%p)", dmaatb);
        /*
         * Initialize the DMAATB structure for new allocation
         */
        memset(dmaatb, 0, sizeof(ve1_dmaatb_reg_t));

        VEOS_TRACE("iterate dma page tables");
        for (dirno = 0; dirno < VE1_DMAATB_DIR_NUM; dirno++) {
                ps_invalid(&(dmaatb->dir[dirno]));
                for (entry = 0; entry < VE1_DMAATB_ENTRY_MAX_SIZE; entry++) {
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_invalidate_dmaatb, ve1_amm_invalidate_dmaatb)

dir_t ve1_amm_validate_vehva(vehva_t vaddr, int rw, pgno_t pgno,
		...
		int numa_node)
{
	...
	if (pgmod == PG_4K) {
		...
		dir_num =  ve1_amm__get_dma_pgd(vaddr, jid, true);
		if (dir_num < 0) {
	...
	ve1_veos_dmaatb_reg_t *pdt= (ve1_veos_dmaatb_reg_t *)mm->dmaatb;
	...
	if (!ps_isvalid(pgd)) {
		...
		ve1__validate_dma_pgd(&pdt->dmaatb_reg,
	...
	if (!((vaddr >= VEHVA_4K_START) && (vaddr < VEHVA_4K_END)))
		VE1_NODE_ARCH_DEP(VE_NODE(0))->hw_dma_map[dir_num].count++;
	VEOS_DEBUG("dma map pgd[%d] count is %lu",
		dir_num, VE1_NODE_ARCH_DEP(VE_NODE(0))->hw_dma_map[dir_num].count);
	...
	dir_num = ve1_amm_sync_node_dmaatb_dir(vaddr, mm, dir_num, jid);
	if (0 > dir_num) {
		VEOS_DEBUG("no page directory for vehva %lx", vaddr);
		ret = dir_num;
		goto error;
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_validate_vehva, ve1_amm_validate_vehva)

dir_t ve1_amm_invalidate_vehva(vehva_t vaddr, struct ve_mm_struct *mm,
				jid_t jid)
{
        uint64_t pgoff = 0;
        pgno_t pb = 0;
        ...
        int pgmod = 0;
        ve1_atb_dir_t *pgd = NULL, *node_pgd = NULL;
        ve1_atb_entry_t *pte, *node_pte = NULL;
        struct ve_node_struct *vnode = VE_NODE(0);
	ve1_veos_dmaatb_reg_t *node_dmaatb = (ve1_veos_dmaatb_reg_t *)vnode->dmaatb;
        int ret;
        ...
        dir_num =  ve1_amm__get_dma_pgd(vaddr, jid, false);
	...
	ve1_veos_dmaatb_reg_t *tsk_dmaatb = (ve1_veos_dmaatb_reg_t *)mm->dmaatb;
        pgmod = ps_getpgsz(&tsk_dmaatb->dmaatb_reg.dir[dir_tsk]);
        pgoff = pgentry(vaddr, pgmod);
        ...
        if (node_pgd) {
                ...
                if (!((vaddr >= VEHVA_4K_START) &&
                                        (vaddr < VEHVA_4K_END))) {
                        VE1_NODE_ARCH_DEP(VE_NODE(0))->hw_dma_map[dir_num].count--;
                        if (!VE1_NODE_ARCH_DEP(VE_NODE(0))->hw_dma_map[dir_num].count) {
                                ps_clrdir(pgd);
                                ps_invalid(pgd);
                                ps_clrdir(node_pgd);
                                ps_invalid(node_pgd);
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_invalidate_vehva, ve1_amm_invalidate_vehva)

int ve1_amm_veos_get_pgmode_vehva(struct ve_task_struct *tsk, uint64_t vaddr)
{
	int ret = 0;
	dir_t dir_num = 0;
	uint64_t pgoff = 0;
	...
	ve1_veos_dmaatb_reg_t *dt = (ve1_veos_dmaatb_reg_t *)tsk->p_ve_mm->dmaatb;
	int jid = tsk->jid;
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_veos_get_pgmode_vehva, ve1_amm_veos_get_pgmode_vehva)
