...
/**
 * @file dmaatb_api.c
 * @brief VE1 dependent part of DMAATB realated functions
 */
#include <libved.h>
#include <ve_hw.h>
...
int64_t ve1_amm_alloc_dmaatb_for_sysregs(struct ve_task_struct *tsk)
{
	veraa_t veraa_paddr[3] = {
		SYSCOM_ADD + offsetof(ve1_system_common_reg_t, STM),
		SYSCOM_ADD + offsetof(ve1_system_common_reg_t, CPMMR),
		0,
	};
	return veos_alloc_dmaatb_entry_for_aa_tsk(tsk, veraa_paddr,
			PROT_READ, VE_ADDR_VERAA,
			VEHVA_SYSCOM_REG | VEHVA_4K, false);
}
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_map_dma_area_to_system_registers, ve1_amm_alloc_dmaatb_for_sysregs)

int ve1_amm_dma_syscall_region_is_mapped(const struct ve_mm_struct *mm)
{
	const ve1_veos_dmaatb_reg_t *dmaatb = (const ve1_veos_dmaatb_reg_t *)mm->dmaatb;
	return ps_isvalid(&dmaatb->dmaatb_reg.dir[0]);
}
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_dma_syscall_region_is_mapped, ve1_amm_dma_syscall_region_is_mapped)

static int ve1_find_aa_in_dmaatb_entries(ve1_atb_entry_t *e, uint64_t paddr,
						uint64_t pagesize, int type)
{
	for (int pg_entry = 0; pg_entry < VE1_DMAATB_ENTRY_MAX_SIZE;
			++pg_entry) {
...
int64_t ve1_amm__aa_to_vehva(const struct ve_mm_struct *mm, uint64_t paddr,
				int type)
{
	ve1_dmaatb_reg_t dmaatb;

	memcpy(&dmaatb,
		&((const ve1_veos_dmaatb_reg_t *)mm->dmaatb)->dmaatb_reg,
		sizeof(ve1_dmaatb_reg_t));

	ve1_atb_dir_t *atbp = dmaatb.dir;
	int pg_entry = -1;
	uint64_t psnum_dir = 0;
	uint64_t pagesize = 0;
	for (int ps_entry = 0; ps_entry < VE1_DMAATB_DIR_NUM; ++ps_entry) {
		/* Is atbp valid ? */
		if (ps_isvalid(atbp)) {
			...
			pg_entry = ve1_find_aa_in_dmaatb_entries(dmaatb.entry[ps_entry],
					paddr, pagesize, type);
			if (pg_entry >= 0) {
	...
	VEOS_DEBUG("ve1_amm__aa_to_vehva fails");
	return -EFAULT;
done:
	return  psnum_dir + (pg_entry * pagesize);

}
VEOS_ARCH_DEP_FUNC(ve1, arch_amm__aa_to_vehva, ve1_amm__aa_to_vehva)

void ve1_amm_free_dmaatb(veos_dmaatb_reg_t *d,
				enum swap_progress dmaatb_progress)
{
	int dir, ent, pg_type, ret;
	ve1_veos_dmaatb_reg_t *ve1_veos_dmaatb_reg = (ve1_veos_dmaatb_reg_t *)d;
	ve1_dmaatb_reg_t *dmaatb = &ve1_veos_dmaatb_reg->dmaatb_reg;
	...
	for (dir = vnode->nr_avail_cores; dir < VE1_DMAATB_DIR_NUM; ++dir) {
		...
		ve1_dmaatb_reg_t *node_dmaatb = &((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg;
		...
		ret = vedl_ve1_update_dmaatb_dir(cnt_regs, node_dmaatb, dir);
		if (ret < 0) {
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_free_dmaatb, ve1_amm_free_dmaatb)

int64_t ve1_amm_update_dmaatb_context(struct ve_task_struct *tsk, int tbl)
{
	...
	struct ve_core_struct *p_ve_core = VE_CORE(0, tbl);
	int phys_core = p_ve_core->phys_core_num;
	VEOS_DEBUG("update_dmaatb_context for pid %d on"
			" table id %d (physical core %d)",
			tsk->pid, tbl, phys_core);
	/* DMACTL */
	paddr[0] = VE1_CORE_SYS_OFFSET(phys_core) +
			VE1_UDMA_OFFSET_OF_DMACTL(0);
	...
	paddr[0] = VE1_CORE_SYS_OFFSET(phys_core) +
			VE1_UDMA_OFFSET_OF_DMADESC(VE_UDMA_EENTRY);
	ret = veos_alloc_dmaatb_entry_for_aa_tsk(tsk, paddr,
			PROT_READ|PROT_WRITE, VE_ADDR_VERAA,
	...
	paddr[0] = VE1_CORE_SYS_OFFSET(phys_core) +
			VE1_UDMA_OFFSET_OF_DMADESC(VE_UDMA_HENTRY);
	ret = veos_alloc_dmaatb_entry_for_aa_tsk(tsk, paddr,
			PROT_READ|PROT_WRITE, VE_ADDR_VERAA,
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_update_dmaatb_context, ve1_amm_update_dmaatb_context)

int ve1_amm_release_dmaatb_entry(struct ve_task_struct *tsk)
{
	int ps_entry = 0;
	ve1_dmaatb_reg_t *dmaatb = &((ve1_veos_dmaatb_reg_t *)tsk->p_ve_mm->dmaatb)->dmaatb_reg;
	struct ve_node_struct *vnode = VE_NODE(0);
	ve1_dmaatb_reg_t *node_dmaatb = &((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg;

	VEOS_TRACE("Invoked ve1_amm_release_dmaatb_entry for pid: %d",
			tsk->pid);

	dir_t dir_num = ve1_amm__get_dma_pgd(tsk->p_ve_mm->shm_lhm_vehva,
					tsk->jid, false);
	if (0 > dir_num) {
	...
	for (ps_entry = 0; ps_entry < VE1_ATB_ENTRY_MAX_SIZE;
			ps_entry++) {
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_release_dmaatb_entry, ve1_amm_release_dmaatb_entry)

int ve1_amm_sync_node_dmaatb_dir(vehva_t vehva, struct ve_mm_struct *mm,
				dir_t dir_idx, jid_t jid)
{
	ve1_atb_dir_t *pgd = NULL;
	ve1_atb_entry_t *pte = NULL;
	dir_t dir_num = 0;
	ve1_dmaatb_reg_t *dmaatb, *dmaatb_node;
	struct ve_node_struct *vnode = VE_NODE(0);
	dmaatb = &((ve1_veos_dmaatb_reg_t *)mm->dmaatb)->dmaatb_reg;
	dmaatb_node = &((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg;
	VEOS_TRACE("Invoked");
	VEOS_DEBUG("vehva %lx dmaatb %p dir_idx %d jid %d",
			vehva, dmaatb, dir_idx, jid);

	if (PG_4K == ps_getpgsz(&dmaatb->dir[dir_idx])) {
		dir_num = ve1_amm__get_dma_pgd(vehva, jid, true);
	...
	memcpy(pte, dmaatb->entry[dir_idx],
		sizeof(ve_reg_t) * VE1_DMAATB_ENTRY_MAX_SIZE);

	return dir_num;
}
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_sync_node_dma_page_dir, ve1_amm_sync_node_dmaatb_dir)

int ve1_amm__map_dmades(struct ve_task_struct *tsk,
...
update_field:
	...
	memset(&TO_VE1_UDMA(tsk->udma_context)->dmades[VE_UDMA_HENTRY], 0,
		sizeof(ve1_dma_desc_t) * VE1_DMA_DSCR_ENTRY_SIZE);
	memset(&TO_VE1_UDMA(tsk->udma_context)->dmactl[VE_UDMA_HENTRY], 0,
	...
	if (set_permission_bit) {
		...
		if (ve1__psm_restore_udma_context(tsk, *(tsk->core_dma_desc),
				VE_UDMA_HENTRY))
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm__map_dmades, ve1_amm__map_dmades)

int64_t ve1_amm_free_dmaatb_entry_tsk(struct ve_task_struct *req_tsk,
						vehva_t vehva, size_t size)
{
	...
	VEOS_TRACE("Invoked");
	VEOS_DEBUG("Invoked(ve1) with pid:%d vehva:%ld size:%ld",
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_free_dmaatb_entry_tsk, ve1_amm_free_dmaatb_entry_tsk)

int64_t ve1_amm_alloc_dmaatb_entry_by_vehva_tsk(struct ve_task_struct *tsk,
		...
		uint64_t vehva_flag, bool halt_all, uint64_t vehva)
{
	...
	dir_t prv_dir = -1;
	dir_t dir_num = -1;
	dir_t dirs[VE1_DMAATB_DIR_NUM] = {0};
	int prot = 0, cnt = 0;
	int64_t count = 0;
	pthread_mutex_t *vehva_lock = NULL; /*!< mutex lock*/
	ve1_dmaatb_reg_t tsk_cp = { { { {0} } } } ,node_cp = { { { {0} } } };
	...
	VEOS_DEBUG("vehva:%ld count:%ld entry:%ld pgmod:%d",
	...
	memset(&tsk_cp, 0, sizeof(ve1_dmaatb_reg_t));
	memset(&node_cp, 0, sizeof(ve1_dmaatb_reg_t));
	/* Here we save the content of node_dmaatb and task_dmaatb.
	 * so in case of failure we can do proper cleanup.
	 */
	memcpy(&tsk_cp, &((ve1_veos_dmaatb_reg_t *)tsk->p_ve_mm->dmaatb)->dmaatb_reg, sizeof(ve1_dmaatb_reg_t));
	memcpy(&node_cp, &((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg, sizeof(ve1_dmaatb_reg_t));
	...
	return ret;
clean_up:
	memcpy(&((ve1_veos_dmaatb_reg_t *)tsk->p_ve_mm->dmaatb)->dmaatb_reg, &tsk_cp, sizeof(ve1_dmaatb_reg_t));
	memcpy(&((ve1_veos_dmaatb_reg_t *)vnode->dmaatb)->dmaatb_reg, &node_cp, sizeof(ve1_dmaatb_reg_t));
...
VEOS_ARCH_DEP_FUNC(ve1, arch_amm_alloc_dmaatb_entry_by_vehva_tsk, ve1_amm_alloc_dmaatb_entry_by_vehva_tsk)
