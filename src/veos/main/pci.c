...
static int veos_handle_pci_common(veos_thread_arg_t *pti, int op)
{
	...
	case REGISTER_TO_PCI_OP:
		...
		veos_sync_r_bar2();
		if (slot < 0){
			VEOS_ERROR("Register to PCI: Fail to map address to PCIATB  0x%lx, size:%zd",
					req.addr, req.size);
			result = slot;
		} else {
	...
	case UNREGISTER_FROM_PCI_OP:
		...
		veos_sync_r_bar2();
		if (result) {
			VEOS_ERROR("Unregister from PCI: Fail to delete pciatb [entry:%d, addr:0x%lx, size:%zd]",
	...
	case REGISTER_TO_DMA_OP:
		...
		veos_sync_r_bar2();
		if (result < 0){
			VEOS_ERROR("Register to DMA: Fail to map 0x%lx to DMAATB, size:%zd",
					req.addr, req.size);
		}
		break;
	...
	case UNREGISTER_FROM_DMA_OP:
		...
		break;
	default:
		VEOS_ERROR("pci request common handling: Invalid operation is specified:%d",
				op);
	}
