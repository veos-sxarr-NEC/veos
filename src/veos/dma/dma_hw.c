...
/**
 * @file dma_hw.c
 * @brief functions to access registers regarding DMA engine
 */
#include <unistd.h>
#include <errno.h>
#include <libved.h>
 ...
 */
int ve_dma_hw_get_readptr(vedl_handle *h, system_common_reg_t *creg)
{
	int rv;
	VE_DMA_TRACE("called");
	vedl_barrier_regs_read(h);
	rv = creg->dmactl.data[1] & VE_DMA_CTL_READ_PTR_MASK;
	return rv;
}
 ...
 */
uint64_t ve_dma_hw_get_ctlword1(vedl_handle *h, system_common_reg_t *creg)
{
	uint64_t rv;
	VE_DMA_TRACE("called");
	vedl_barrier_regs_read(h);
	rv = creg->dmactl.data[1] &
	     (VE_DMA_CTL_EXCEPTION_MASK | VE_DMA_CTL_READ_PTR_MASK);
	return rv;
}
 ...
 */
uint32_t ve_dma_hw_get_ctlstatus(vedl_handle *h, system_common_reg_t *creg)
{
	uint32_t rv;
	VE_DMA_TRACE("called");
	vedl_barrier_regs_read(h);
	rv = creg->dmactl.data[0] & VE_DMA_CTL_STATUS_MASK;
	return rv;
}
 ...
 */
static int ve_dma_hw_check_addr_type(const char *msg, uint64_t t)
{
	VE_DMA_TRACE("called");
	switch (t) {
		       ...
		       uint64_t dstaddr, uint32_t len, int sync)
{
	...
	if (ve_dma_hw_check_addr_type("source", srctype) != 0) {
		/* error message is output in ve_dma_hw_check_addr_type(). */
		return -EINVAL;
	}
	if (ve_dma_hw_check_addr_type("destination", dsttype) != 0) {
		/* error message is output in ve_dma_hw_check_addr_type(). */
		return -EINVAL;
	}
	if (entry < 0 || entry >= VE_DMA_NUM_DESC) {
		VE_DMA_ERROR("Descriptor number %d is invalid", entry);
		return -EINVAL;
	}

	/* write DMA descriptor */
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMADESC(entry), 0);
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMADESC(entry) + 8,
	...
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMADESC(entry) + 0x10,
			      srcaddr);
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMADESC(entry) + 0x18,
			      dstaddr);
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMADESC(entry),
			      VE_DMA_DESC_STATUS_PREPARED);
 ...
 * @param[in,out] creg pointer to node control register area
 */
void ve_dma_hw_start(vedl_handle *h, system_common_reg_t *creg)
{
	VE_DMA_TRACE("called");
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMACTL,
			      VE_DMA_CTL_PERM_START);
}
 ...
 * @param[in,out] creg pointer to node control register area
 */
void ve_dma_hw_post_stop(vedl_handle *h, system_common_reg_t *creg)
{
	VE_DMA_TRACE("called");
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMACTL, 0);
}
 ...
 */
uint64_t ve_dma_hw_desc_status(vedl_handle *h, system_common_reg_t *creg,
			       int entry)
{
	dma_desc_t *dp;
	uint64_t word;
	...
	VE_DMA_ASSERT(0 <= entry && entry < VE_DMA_NUM_DESC);

	dp = creg->dmadesc + entry;
	vedl_barrier_regs_read(h);
	word = dp->data[0];
 ...
 * @param entry index of DMA descriptor
 */
void ve_dma_hw_nullify_dma(vedl_handle *h, system_common_reg_t *creg, int entry)
{
	VE_DMA_TRACE("called (entry=%d)", entry);
...
/**
 * @brief Clear DMA descriptor
 *
 * @param[in,out] h VEDL handle
 * @param[in,out] creg pointer to node control register area
 * @param entry index of DMA descriptor to clear
 *
 * @return 0 on success. Current implementation always returns 0..
 */
int ve_dma_hw_clear_dma(vedl_handle *h, system_common_reg_t *creg, int entry)
{
	VE_DMA_TRACE("called (entry=%d)", entry);
	...
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_DMADESC(entry), 0);
	VE_DMA_TRACE("DMA desc %d word 0 is cleared.", entry);
	return 0;
}
 ...
 * @return 0 on success. Current implementation always returns 0.
 */
int ve_dma_hw_set_readptr(vedl_handle *h, system_common_reg_t *creg, int entry)
{
	uint64_t word;
	VE_DMA_TRACE("called (entry=%d)", entry);
	VE_DMA_ASSERT(0 <= entry && entry < VE_DMA_NUM_DESC);
	...
	word = entry;
	vedl_set_cnt_reg_word(h, creg, VE_DMA_OFFSET_OF_WORD_READPTR, word);
