						...
						veos_dmaatb_reg_t *sync_copy_d)
{
	...
	int ret;
	pgno_t pgno = 0;
	ve3_veos_dmaatb_reg_t *ve3_veos_dmaatb_reg = (ve3_veos_dmaatb_reg_t *)sync_copy_d;
	ve3_dmaatb_reg_t *sync_copy = &ve3_veos_dmaatb_reg->dmaatb_reg;
	...
	if (!ps_isvalid(&(sync_copy->dir[dir]))) {
		...
		goto hndl_return;
	} else if (ent == (VE3_DMAATB_ENTRY_MAX_SIZE - 1)) {
 ...
 * @retval -1 on Failure.
 */
int ve3_pps_veos_swap_out_dmaatb(struct ve_task_struct *tsk)
{
	int retval = 0;
	int dir, ent, ret;
	uint64_t sync_dir_bitmap[VE3_DMAATB_DIR_NUM/64] = {};
	veos_dmaatb_reg_t *sync_copy = NULL;
	ve3_dmaatb_reg_t mm_dmaatb = {};
	struct ve_node_struct *vnode = VE_NODE(0);
	...
	for (dir = vnode->nr_avail_cores; dir < VE3_DMAATB_DIR_NUM; dir++) {
		for (ent = 0; ent < VE3_DMAATB_ENTRY_MAX_SIZE; ent++) {
			...
			sync_dir_bitmap[dir/64] |= (1ULL << (dir - (dir/64)*64));
			pthread_mutex_lock_unlock(
				&(tsk->p_ve_mm->thread_group_mm_lock),
				LOCK, "Failed to acquire thread_group_mm_lock");
	...
	for (dir = vnode->nr_avail_cores; dir < VE3_DMAATB_DIR_NUM; dir++) {
		if (!(sync_dir_bitmap[dir/64] & (1ULL << (dir - (dir/64)*64))))
			continue;
		PPS_DEBUG(cat_os_pps, "Syncing DMAATB Dir %d", dir);
		ve3_veos_dmaatb_reg_t *ve3_veos_dmaatb_reg = (ve3_veos_dmaatb_reg_t *)sync_copy;
		ve3_dmaatb_reg_t *sync_copy_dmaatb = &ve3_veos_dmaatb_reg->dmaatb_reg;
...
VEOS_ARCH_DEP_FUNC(ve3, arch_pps_veos_swap_out_dmaatb, ve3_pps_veos_swap_out_dmaatb)
