...
/**
 * @file mm_stat.c
 * @brief VE3 dependent part of command handlers
 *
 * @internal
 * @author AMM
 */
...
int ve3_amm_get_VEHVA_swappable_size(veos_dmaatb_reg_t *reg,
				unsigned long *swappable_size_p)
{
	struct ve_node_struct *vnode = VE_NODE(0);
	ve3_dmaatb_reg_t *dmaatb = &((ve3_veos_dmaatb_reg_t *)reg)->dmaatb_reg;
	size_t tmp_swapped_size = 0;

	for (int dir = vnode->nr_avail_cores; dir < VE3_DMAATB_DIR_NUM; ++dir) {
		VEOS_DEBUG("DIR[%d]: 0x%lx", dir, dmaatb->dir[dir].data);
		for (int ent = 0; ent < VE3_DMAATB_ENTRY_MAX_SIZE; ++ent) {
...
VEOS_ARCH_DEP_FUNC(ve3, arch_amm_get_dma_space_swappable_size, ve3_amm_get_VEHVA_swappable_size)
