...
/*
 * @file vehva_mgmt.c
 * @brief VE3 dependent part of VEHVA management related function.
 *
 * @internal
 * @author AMM
 */
...
int ve3_amm_vehva_init(struct ve_mm_struct *vm)
{
	int ret = 0;
	/*
	...
	vm->vehva_header.bmap_256m = (uint64_t *)calloc(1, DMA_BITMAP);
	if (vm->vehva_header.bmap_256m == NULL) {
		ret = -errno;
		VEOS_CRIT("Error (%s) whiel allocating bmap_256M", strerror(-ret));
		goto free_bmap_64m;
	}

	memset(vm->vehva_header.bmap_256m, 0xff, DMA_BITMAP);
	...
	pthread_mutex_init(&vm->vehva_header.vehva_256m_lock, NULL);
...
VEOS_ARCH_DEP_FUNC(ve3, arch_amm_vehva_init, ve3_amm_vehva_init)

int64_t ve3_amm_vehva_alloc(int flag, uint64_t size, struct ve_mm_struct *mm)
{
	...
	if (flag & VEHVA_4K) {
		...
		} else if (flag & VEHVA_256MB) {
			VEOS_DEBUG("VEHVA request type VESHM_256MB");
			pgsz = PAGE_SIZE_256MB;
			vehva_base = VEHVA_256M_START;
			vehva_lock = &mm->vehva_header.vehva_256m_lock;
			bmap = mm->vehva_header.bmap_256m;
		} else {
			ret = -EINVAL;
...
VEOS_ARCH_DEP_FUNC(ve3, arch_amm_vehva_alloc, ve3_amm_vehva_alloc)

int ve3_amm_vehva_free(vehva_t vehva_start, size_t size,
		struct ve_task_struct *tsk)
{
	...
	if ((vehva_start >= VEHVA_4K_START) &&
		...
		pgmod = PG_HP;
	} else if ((vehva_start >= VEHVA_256M_START) &&
			(vehva_end <= VEHVA_256M_END)) {
		VEOS_DEBUG("Freeing VEHVA_256M Region");
		count = size/(PAGE_SIZE_64MB * 4);
		bmap = tsk->p_ve_mm->vehva_header.bmap_256m;
		vehva_lock = &tsk->p_ve_mm->vehva_header.vehva_256m_lock;
		entry = (vehva_start - VEHVA_256M_START)/(PAGE_SIZE_256MB);
		pgmod = PG_SHP;
	} else {
		ret = -EINVAL;
...
VEOS_ARCH_DEP_FUNC(ve3, arch_amm_vehva_free, ve3_amm_vehva_free)
