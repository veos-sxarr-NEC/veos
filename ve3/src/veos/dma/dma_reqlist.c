 ...
 */
int ve3_dma_addrtype_to_pagesize(ve_dma_addrtype_t t)
{
...
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_addrtype_to_pagesize, ve3_dma_addrtype_to_pagesize)

int ve3_dma_addrtype_to_hwtype(ve_dma_addrtype_t t)
{
	switch (t) {
	case VE_DMA_VHVA:
	case VE_DMA_VHSAA:
	...
	case VE3_DMA_VERAA_NON_PREFETCHABLE:
		return VE_DMA_DESC_ADDR_VERAA_NON_PREFETCHABLE;
	case VE3_DMA_VERAA_PREFETCHABLE:
		return VE_DMA_DESC_ADDR_VERAA_PREFETCHABLE;
	default:
		veos_abort("unexpected addrtype %d", t);
	}
}
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_addrtype_to_hwtype, ve3_dma_addrtype_to_hwtype)
 ...
 */
int ve3_dma_pin_blk(vedl_handle *vh, pid_t pid, ve_dma_addrtype_t t, int wr,
			struct ve_dma__block *blk)
{
	...
	case VE3_DMA_VERAA_PREFETCHABLE:
		blk->unpin = NULL;
		blk->pgsz = ve3_dma_addrtype_to_pagesize(t);
...
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_pin_blk, ve3_dma_pin_blk)
 ...
 */
int ve3_dma_request_mergeable(const ve_dma_reqlist_entry *e,
...
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_request_mergeable, ve3_dma_request_mergeable)
 ...
 *
 */
int ve3_dma_reqlist__entry_post(ve_dma_reqlist_entry *e)
{
	ve_dma_hdl *hdl;
	int entry;
	int ret;
	...
	struct ve3_dma_engine_usage *usage = VE3_DMA_HDL_USAGE(hdl);
	VE_DMA_TRACE("%d entries are used", usage->desc_num_used);
	if (usage->desc_num_used >= (VE3_DMA_NUM_DESC - 1)) {
	...
	if ((!is_sync) && ((e->last)
		|| (usage->desc_num_used == (VE3_DMA_NUM_DESC - 2))))
		is_sync = 1;

	ret = ve3_dma_hw_post_dma(hdl->vedl_handle, hdl->control_regs, entry,
...
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_reqlist__entry_post, ve3_dma_reqlist__entry_post)
 ...
 */
void ve3_dma_free_used_desc(ve_dma_hdl *hdl, int readptr)
{
	struct ve3_dma_engine_usage *u = VE3_DMA_HDL_USAGE(hdl);
	while (u->req_entry[u->desc_used_begin] == NULL &&
		...
		--u->desc_num_used;
		++u->desc_used_begin;
		if (u->desc_used_begin == VE3_DMA_NUM_DESC)
			u->desc_used_begin = 0;
	}
}
 ...
 * @param readptr Read pointer value of DMA control register
 */
void ve3_dma_finish_reqlist_entry(ve_dma_reqlist_entry *e, uint64_t status,
				 int readptr)
{
	...
	e->entry = -1;
	struct ve3_dma_engine_usage *usage = VE3_DMA_HDL_USAGE(dh);
	usage->req_entry[entry] = NULL;
	VE_DMA_TRACE("desc_used_begin = %d, desc_num_used = %d",
		     usage->desc_used_begin, usage->desc_num_used);
	ve3_dma_free_used_desc(dh, readptr);
	VE_DMA_TRACE("desc_used_begin = %d, desc_num_used = %d",
		     usage->desc_used_begin, usage->desc_num_used);
}
 ...
 */
void ve3_dma_reqlist__cancel(ve_dma_req_hdl *req)
{
	/*
	...
	VE_DMA_TRACE("called");
	int readptr;
	readptr = ve3_dma_hw_get_readptr(vh, hdl->control_regs);
	...
	list_for_each_safe(p, tmp, &req->reqlist) {
		...
		if (e->status < VE_DMA_ENTRY_ONGOING) {
			...
			VE_DMA_TRACE("request %p is on descriptor #%d",
				     e, e->entry);
			if (e->entry < 0 || e->entry >= VE3_DMA_NUM_DESC) {
				VE_DMA_CRIT("descriptor #%d is invalid",
					    e->entry);
				return;
			}
			uint64_t status_hw;
			status_hw = ve3_dma_hw_desc_status(vh,
							  hdl->control_regs,
							  e->entry);
			if (status_hw & VE_DMA_DESC_STATUS_PREPARED) {
				ve3_dma_hw_nullify_dma(hdl->vedl_handle,
					hdl->control_regs, e->entry);
			}
			ve3_dma_finish_reqlist_entry(e, status_hw, readptr);
		} else {
...
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_reqlist_cancel, ve3_dma_reqlist__cancel)
