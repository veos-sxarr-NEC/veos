 ...
 */
static ve_dma_reqlist_entry *ve3_dma_intr__finish_descriptor(struct ve3_dma_engine_usage *usage,
			int entry, uint64_t status, int readptr)
{
	...
	if (e != NULL) {
		...
		if (status & VE_DMA_DESC_EXCEPTION_MASK) {
			...
			ve3_dma_finish_reqlist_entry(e, status, readptr);
			if (e->last)
				r->comp = 1;
			ret = e;
		}
 ...
 *
 */
void ve3_dma_intr(ve_dma_hdl *dh)
{
	int exc, entry, readptr, count, i;
	uint64_t ctlword1;
	vedl_handle *handle = dh->vedl_handle;
	ve_dma_reqlist_entry *e;
        ve_dma__block *b[VE3_DMA_NUM_DESC * 2];

	pthread_mutex_lock(&dh->mutex);
	VE_DMA_TRACE("DMA control register status = 0x%x",
		     ve3_dma_hw_get_ctlstatus(handle, dh->control_regs));

	/* get exc and readptr */
	exc = 0;
	ctlword1 = ve3_dma_hw_get_ctlword1(handle, dh->control_regs);
	...
	if (exc == 0) {
		...
		for (entry = current_begin; entry != readptr;
		     entry = (entry + 1) % VE3_DMA_NUM_DESC) {
			e = ve3_dma_intr__finish_descriptor(usage,
		...
		for (entry = current_begin; entry != readptr;
		     entry = (entry + 1) % VE3_DMA_NUM_DESC) {
			uint64_t status;
			status = ve3_dma_hw_desc_status(dh->vedl_handle,
					dh->control_regs, entry);
			VE_DMA_ASSERT(status & VE_DMA_DESC_STATUS_COMPLETED);
			e = ve3_dma_intr__finish_descriptor(usage, entry, status,
							readptr);
			if (e) {
				if (e->src_block) {
					b[count++] = e->src_block;
		...
		ve3_dma_free_used_desc(dh, readptr);
		/* clear exc bit */
		ve3_dma_hw_set_readptr(handle, dh->control_regs,
				      readptr);
		/* restart DMA engine */
...
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_intr, ve3_dma_intr)
