...
/**
 * @file dma_hw.c
 * @brief VE3-spacific functions to access registers regarding DMA engine
 */
 ...
 */
int ve3_dma_hw_get_readptr(vedl_handle *h, vedl_common_reg_handle *creg)
{
	ve_reg_t reg;
	VE_DMA_TRACE("called");
	vedl_barrier_regs_read(h);
	vedl_get_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_WORD_READPTR, &reg);
 ...
 */
uint64_t ve3_dma_hw_get_ctlword1(vedl_handle *h, vedl_common_reg_handle *creg)
{
	ve_reg_t reg;
	VE_DMA_TRACE("called");
	vedl_barrier_regs_read(h);
 ...
 */
uint32_t ve3_dma_hw_get_ctlstatus(vedl_handle *h, vedl_common_reg_handle *creg)
{
	ve_reg_t reg;
	VE_DMA_TRACE("called");
	vedl_barrier_regs_read(h);
	vedl_get_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_DMACTL, &reg);
 ...
 */
static int ve3_dma_hw_check_addr_type(const char *msg, uint64_t t)
{
	VE_DMA_TRACE("called");
	switch (t) {
	case VE_DMA_DESC_ADDR_VEMAA:
		...
		return 0;
	case VE_DMA_DESC_ADDR_VERAA_PREFETCHABLE:
		VE_DMA_TRACE("%s addr type is VERAA (prefetchable)", msg);
		return 0;
	case VE_DMA_DESC_ADDR_VERAA_NON_PREFETCHABLE:
		VE_DMA_TRACE("%s addr type is VERAA (non-prefetchable)", msg);
		return 0;
 ...
 * @return 0 on success. Non-zero on failure.
 */
int ve3_dma_hw_post_dma(vedl_handle *h, vedl_common_reg_handle *creg, int entry,
		       ...
		       uint64_t dstaddr, uint32_t len, int sync, int ro, int ido)
{
	...
	if (ve3_dma_hw_check_addr_type("source", srctype) != 0) {
		/* error message is output in ve_dma_hw_check_addr_type(). */
		return -EINVAL;
	}
	if (ve3_dma_hw_check_addr_type("destination", dsttype) != 0) {
		/* error message is output in ve_dma_hw_check_addr_type(). */
		return -EINVAL;
	}
	if (entry < 0 || entry >= VE3_DMA_NUM_DESC) {
		VE_DMA_ERROR("Descriptor number %d is invalid", entry);
		return -EINVAL;
	}

	/* write DMA descriptor */
	vedl_set_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_DMADESC(entry), 0);
	vedl_set_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_DMADESC(entry) + 8,
 ...
 * @param[in,out] creg pointer to node control register area
 */
void ve3_dma_hw_start(vedl_common_reg_handle *creg)
{
	VE_DMA_TRACE("called");
	vedl_set_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_DMACTL, VE_DMA_CTL_PERM_START);
}
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_hw_start, ve3_dma_hw_start)
 ...
 * @param[in,out] creg pointer to node control register area
 */
void ve3_dma_hw_post_stop(vedl_common_reg_handle *creg)
{
	VE_DMA_TRACE("called");
	vedl_set_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_DMACTL, 0);
}
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_hw_post_stop, ve3_dma_hw_post_stop)
 ...
 */
uint64_t ve3_dma_hw_desc_status(vedl_handle *h, vedl_common_reg_handle *creg,
				int entry)
{
	ve_reg_t word;
	VE_DMA_TRACE("called (entry=%d)", entry);

	VE_DMA_ASSERT(0 <= entry && entry < VE3_DMA_NUM_DESC);

	vedl_barrier_regs_read(h);
	vedl_get_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_DMADESC(entry), &word);
 ...
 * @param entry index of DMA descriptor
 */
void ve3_dma_hw_nullify_dma(vedl_handle *h, vedl_common_reg_handle *creg,
				int entry)
{
	VE_DMA_TRACE("called (entry=%d)", entry);
 ...
 */
int ve3_dma_hw_clear_dma(vedl_handle *h, vedl_common_reg_handle *creg)
{
	VE_DMA_TRACE("called");

	for (int i = 0; i < VE3_DMA_NUM_DESC; ++i)
		vedl_set_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
			VE_DMA_OFFSET_OF_DMADESC(i), 0);
	return 0;
}
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_hw_clear_all_dma, ve3_dma_hw_clear_dma)
 ...
 * @return 0 on success. Current implementation always returns 0.
 */
int ve3_dma_hw_set_readptr(vedl_handle *h, vedl_common_reg_handle *creg,
				int entry)
{
	ve_reg_t word;
	VE_DMA_TRACE("called (entry=%d)", entry);
	...
	word = entry;
	vedl_set_cnt_reg_word(creg, LIBVED_VE3_AREA_BAR4,
		VE_DMA_OFFSET_OF_WORD_READPTR, word);
	VE_DMA_TRACE("DMA read ptr is set to %ld", word);
	return 0;
}

int ve3_dma_hw_engine_is_halt(vedl_handle *h, vedl_common_reg_handle *creg)
{
	uint32_t ctl_status = ve3_dma_hw_get_ctlstatus(h, creg);
	return ctl_status == VE_DMA_CTL_STATUS_HALT;
}
VEOS_ARCH_DEP_FUNC(ve3, arch_dma_hw_engine_is_halt, ve3_dma_hw_engine_is_halt)
