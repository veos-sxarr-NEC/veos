...
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_get_stack_pointer_from_core, ve3_psm_get_SP_from_core)

ve_reg_t ve3_psm_get_SP_on_thread_struct(const void *u)
{
	const ve3_core_user_reg_pre_t *user_regs = u;
	return user_regs->SR[11];
}
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_get_stack_pointer_on_thread_struct, ve3_psm_get_SP_on_thread_struct)
...
*/
int ve3_psm_restore_ve_context(struct ve_task_struct *ve_task_curr)
{
	int64_t frame_v_addrs;
	ve_reg_t curr_sp;
	struct ve3_sigframe p_ve_sigframe;
	ve_dma_hdl *dh;
	ve_dma_status_t st;
	...
	if (ve_task_curr->p_ve_core->curr_ve_task == ve_task_curr) {
		if (ve3_psm_get_SP_from_core(ve_task_curr->core_id, &curr_sp)) {
			veos_abort("failed to get user registers");
		}
	} else {
		curr_sp = ve3_psm_get_SP_on_thread_struct(ve_task_curr->p_ve_thread->arch_user_regs);
	}
	pthread_rwlock_lock_unlock(&(ve_task_curr->p_ve_core->ve_core_lock),
...
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_restore_ve_context_from_sighand, ve3_psm_restore_ve_context)

void *ve3_psm_get_ICE_on_thread_struct(const void *u)
{
	const ve3_core_user_reg_pre_t *user_regs = u;
	return (void *)user_regs->ICE;
}
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_get_exception_addr_on_thread_struct, ve3_psm_get_ICE_on_thread_struct)
...
*/
static int ve3_psm_getframe(struct ve_task_struct *p_ve_task,
				...
				int *on_altstack, int *pg_mode)
{
	...
	ve3_core_user_reg_pre_t *user_regs = p_ve_task->p_ve_thread
						->arch_user_regs;
	if (!onsigstack) {
	...
	if (aligned_addr - frame_vir_addrs < sizeof(struct ve3_sigframe)) {
...
*/
int ve3_psm_setup_ve_frame(int signum,
		...
		int flag)
{
	...
	struct ve3_sigframe ve_sigframe;
	...
	memset(&ve_sigframe, '\0', sizeof(struct ve3_sigframe));

	pthread_mutex_lock_unlock(&(p_ve_task->ve_task_lock), LOCK
		, "failed to acquire task lock");

	ret = ve3_psm_getframe(p_ve_task, signum, &frame_vir_addr,
	...
	memcpy(&(ve_sigframe.uc.uc_mcontext),
			...
			sizeof(ve3_core_user_reg_pre_t));
	ve_sigframe.uc.uc_mcontext.Padding_exs = p_ve_task->p_ve_thread->EXS;
	...
	for (i = 0; frame_addrs[i]; i++) {
		if (i == 0) {
			local_offset = 0;
			dma_size = offset ? offset : (uint64_t)sizeof(struct ve3_sigframe);
		} else {
			local_offset = offset;
			dma_size = (uint64_t)sizeof(struct ve3_sigframe) - offset;
		}
	...
	user_regs->SR[1] = frame_vir_addr +
		offsetof(struct ve3_sigframe, ve_siginfo);
	user_regs->SR[2] = frame_vir_addr + offsetof(struct ve3_sigframe, uc);
...
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_setup_ve_frame, ve3_psm_setup_ve_frame)

ve_reg_t ve3_psm_get_SR0_on_thread_struct(const void *u)
{
	const ve3_core_user_reg_pre_t *user_regs = u;
	return user_regs->SR[0];
}
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_get_retval_on_thread_struct, ve3_psm_get_SR0_on_thread_struct)

void ve3_psm_set_SR0_on_thread_struct(void *u, ve_reg_t val)
{
	ve3_core_user_reg_pre_t *user_regs = u;
	user_regs->SR[0] = val;
}
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_set_retval_on_thread_struct, ve3_psm_set_SR0_on_thread_struct)

void ve3_psm_rewind_IC_on_thread_struct(void *u)
{
	ve3_core_user_reg_pre_t *user_regs = u;
	user_regs->IC -= 8;
}
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_set_program_counter_to_restart, ve3_psm_rewind_IC_on_thread_struct)
