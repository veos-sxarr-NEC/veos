 ...
 *
 * @return,
 * -2 if received version number is smallar than 3
 ...
 */
int ve3_psm_comm_version_compare(char *veos_ver, char *recvd_ver)
{
	...
	for (vlv = 0, lv = 0, position = 0; (vlv < veos_len || lv < recvd_len);position++) {
		...
		if (recvd_num < veos_num) {
			...
			if(recvd_major < 3){
				VEOS_DEBUG("veos version is 3, but received version is under 3");
				return -2;
			}
			return 1;
		}
...
VEOS_ARCH_DEP_FUNC(ve3, arch_psm_comm_version_compare, ve3_psm_comm_version_compare)
