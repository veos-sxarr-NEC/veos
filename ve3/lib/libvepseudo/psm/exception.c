...
/**
* @file	exception.c
* @brief VE3-dependent part of exception handling
*
* @internal
...
*/
int ve3_exception_to_signal(uint64_t exception, siginfo_t *siginfo)
{
...
VEOS_ARCH_DEP_FUNC(ve3, arch_exception_to_signal, ve3_exception_to_signal)
...
*/
void ve3_exception_handler(veos_handle *handle, uint64_t exs)
{
	uint64_t ve_hw_exception = -1;
	int64_t ret;
	ve_reg_t exs_delayed = 0;
	bool check_exs_again = false;
	...
	if (exs & EXS_MONC && 315 == vedl_get_syscall_num(handle->ve_handle)) {
		PSEUDO_DEBUG("MONC for grow(), check EXS again for delayed exception");
		ts.tv_nsec = 230;
		check_exs_again = true;
	} else if (exs & (EXS_ADRM|EXS_BRTR|EXS_STEP|EXS_RDBG)) {
		PSEUDO_DEBUG("Correctable exception, check EXS again for delayed exception");
		ts.tv_nsec = 400;
		check_exs_again = true;
	}
	if (check_exs_again) {
wait_again:
		...
		vedl_get_exs_register(handle->ve_handle, &exs_delayed);
		if (exs_delayed)
			exs |= exs_delayed;
	}
...
VEOS_ARCH_DEP_FUNC(ve3, arch_exception_handler, ve3_exception_handler)
